<table class="table datatable">
                ...
                <td>
                    <select class="form-control branchesvalue4" name="country[]" id="branchesvalue4" @if ($errors->has('country.'.$i)) style="border-color: red;" @endif  data-validate-length-range="1" data-validate-words="1" required="required">
                        ...
                        @foreach($countries as $country)
                            <option value="{{ $country->id }}" @if ($country->name==Input::old('country')[$i]) selected="selected" @endif>{{ $country->name }}</option>
                        @endforeach
                    </select>
                </td>
                <td>
                    ...
                    @else
                        ...
                        <input type="hidden" class="form-control branchesvalue4" id="branchesvalue5" name="id[]" value="{{Input::old('id')[$i]}}">
    ...
    @elseif(sizeof($branches)>0)

        @foreach($branches as $branch)
            <tr class="controls" >
                <td class="item form-group"><input type="text"  class="branchesvalue form-control branchesvalue1" id="branchesvalue1" name="branch_name[]" placeholder="Branch Name" value="{{$branch->hcf_branch_name}}" data-validate-length-range="1" data-validate-words="1" required="required"></td>
                <td class="item form-group"><input type="text" class="branchesvalue form-control branchesvalue2" id="branchesvalue2"  name="branch_address[]" placeholder="Branch Address" value="{{$branch->hcf_branch_address}}" data-validate-length-range="1"  required="required"></td>
                <td>
                    <select class="form-control branchesvalue3"  name="location[]" id="branchesvalue3" data-validate-length-range="1" data-validate-words="1" required="required">
                        <option value="">Select a City</option>
                        @foreach($cities as $city)
                            <option value="{{$city->id}}" @if ($city->city_name==$branch->city_name) selected="selected" @endif>{{$city->city_name }}</option>
                        @endforeach
                    </select>

                </td>
                <td>
                    <select class="form-control branchesvalue4"  name="country[]" id="branchesvalue4" data-validate-length-range="1" data-validate-words="1" required="required" disabled="true">
                        <option value="">Select a country</option>
                        @foreach($countries as $country)
                            <option value="{{ $country->id }}" @if ($country->name==$branch->name) selected="selected" @endif>{{ $country->name }}</option>
                        @endforeach
                    </select>
                </td>
    ...
    @else
        ...
        @else
            <tr class="controls" >
                ...
                <td><input class="form-control branchesvalue5" name="id[]" id="branchesvalue5" value="0" type="hidden"></td>
            </tr>
        @endif
