...
class RouteCollectionBuilderTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testFlushOrdering()
    {
        ...
        // 4) Add another route
        $routes->add('/adm', 'AppBundle:Admin:dashboard', 'admin_dashboard');
    ...
    public function testFlushSetsRouteNames()
    {
        ...
        // add a "named" route
        $collectionBuilder->add('/adm', 'AppBundle:Admin:dashboard', 'admin_dashboard');
    ...
    public function testFlushSetsPrefixedWithMultipleLevels()
    {
        ...
        // embedded collection under /adm
        $adminBlogRoutes = $routes->createBuilder();
        $adminBlogRoutes->add('/new', 'BlogController::newAction', 'admin_blog_new');
        // mount into adm, but before the parent collection has been mounted
        $adminRoutes->mount('/blog', $adminBlogRoutes);

        // now mount the /adm routes, above should all still be /blog/adm
        $routes->mount('/adm', $adminRoutes);
        ...
        $loader
            ...
            ->method('load')
            ->will($this->returnValue($importedCollection));
        // import this from the /adm route builder
        $adminRoutes->import('adm.yml', '/imported');

        $collection = $routes->build();
        $this->assertEquals('/adm/dashboard', $collection->get('admin_dashboard')->getPath(), 'Routes before mounting have the prefix');
        $this->assertEquals('/adm/users', $collection->get('admin_users')->getPath(), 'Routes after mounting have the prefix');
        $this->assertEquals('/adm/blog/new', $collection->get('admin_blog_new')->getPath(), 'Sub-collections receive prefix even if mounted before parent prefix');
        $this->assertEquals('/adm/stats/sales', $collection->get('admin_stats_sales')->getPath(), 'Sub-collections receive prefix if mounted after parent prefix');
        $this->assertEquals('/adm/imported/foo', $collection->get('imported_route')->getPath(), 'Normal RouteCollections are also prefixed properly');
    }
